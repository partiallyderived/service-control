...
class SlackCommandsService(Service):
    ...
    SCHEMA: Final[JSONType] = {
        ...
        'properties': {
            'root': {
                'description':
                    'Fully-qualified name of the :class:`.CommandGroup` to use '
                    'as the root for commands.',
                'type': 'string'
            }
        },
        'required': ['root'],
    ...
    def __init__(
        self, config: JSONType, bolt: App, log: Logger | LoggerAdapter
    ) -> None:
        ...
        :raise TypeError: If ``not isinstance(config['root'], CommandGroup``.
        """
        super().__init__(config)
        root = enough.import_object(config['root'])
        if not isinstance(root, CommandGroup):
            raise TypeError(
                f'Root object {root} (found at {config["root"]}) is not an '
                f'instance of CommandGroup.'
            )
        self._state = SlackCommandsState(
            config['name'], root, bolt=bolt, log=log
        )
        self._handler = SlackCommandsHandler()

    def start(self) -> None:
        """Start the service by registering the handler with the Slack bolt
        application.
        """
