...
from enough.exceptions import EnoughError, EnumErrorsErrors


# These classes are used in test_enum_errors but are defined outside of it to
# avoid issues with '<locals>' being part of the qualified path to created
# errors, which messes with the expected __str__ and __repr__ results. Also note
# that the below classes are deliberately named so that they do not start with
# "Test", as otherwise Pytest would try to use them as test classes.
...
def test_enum_errors() -> None:
    ...
    assert EnumErrorsErrors.error_type() is EnoughError
    ...
    # Test that __name__ and __qualname__ are correctly set for the class
    # (they are set dynamically).
    assert ErrorsToTest.Error1.__name__ == 'Error1'
    assert (
        ErrorsToTest.Error1.__qualname__
        == f'{ErrorsToTest.__qualname__}.Error1'
    )

    # Verify correct naming of the dynamically created __str__ method.
    assert ErrorsToTest.Error1.__str__.__name__ == '__str__'
    assert (
        ErrorsToTest.Error1.__str__.__qualname__
        == f'{ErrorsToTest.Error1.__qualname__}.__str__'
    )
    ...
    exc3 = ErrorsToTest.Error3(args='args', value='value')
    ...
    # noinspection PyTypeChecker
    with enough.raises(EnumErrorsErrors.NotType) as exc_info:
        # noinspection PyUnusedLocal
    ...
    # noinspection PyTypeChecker
    with enough.raises(EnumErrorsErrors.NotExcType) as exc_info:
        # noinspection PyUnusedLocal
...
def test_collect_errors() -> None:
    # Test that an exception type can use collect_errors to catch multiple
    # exceptions while mapping values in a collection and raise itself with a
    # keyword argument set to a mapping from any values for which the mapping
    # function raised an exception of the specified type to the raised
    # exception.
    ...
    with enough.raises(
        TestErrors.Error(exceptions={'arg1': err1, 'arg3': err2}, val='val')
    ):
    ...
    m.side_effect = side_effects
    with enough.raises(TestErrors.Error(exceptions={}, val='val')):
        TestErrors.Error.collect_errors(
            ('arg1', 'arg2', 'arg3'),
            m,
            (ValueError, TypeError),
    ...
    # Specifying key_fn allows us to choose how the resulting error mapping is
    # constructed.
    m.side_effect = side_effects
    with enough.raises(
        TestErrors.Error(exceptions={'ARG1': err1, 'ARG3': err2}, val='val')
...
def test_wrap() -> None:
    # Test that an exception type can use wrap to catch a particular EnumErrors
    # enum and raise itself instead, optionally copying over some of the
    ...
    with enough.raises(TestEnumErrors2.Error2(attr1=1, attr2=2, attr3=3)):
    ...
    with enough.raises(TestEnumErrors3.Error3(attr1=1, attr2=2)):
        with TestEnumErrors2.Error2.wrap(TestEnumErrors1.Error1, attr3=3):
            raise TestEnumErrors3.Error3(attr1=1, attr2=2)

    # Same result even if the differing types are from different enumerations.
    with enough.raises(TestEnumErrors1.Error1_2(attr1=1, attr2=2)):
        with TestEnumErrors2.Error2.wrap(TestEnumErrors1.Error1, attr3=3):
            raise TestEnumErrors1.Error1_2(attr1=1, attr2=2)

    # forward=False should result in no attributes being forwarded.
    with enough.raises(TestEnumErrors2.Error2(attr1=1, attr2=2, attr3=3)):
        with TestEnumErrors2.Error2.wrap(
            TestEnumErrors1.Error1, forward=False, attr1=1, attr2=2, attr3=3
        ):
            raise TestEnumErrors1.Error1(attr1=4, attr2=5)

    # Giving an iterable for forward should result in only those attributes
    # being forwarded.
    with enough.raises(TestEnumErrors2.Error2(attr1=1, attr2=5, attr3=3)):
        with TestEnumErrors2.Error2.wrap(
            TestEnumErrors1.Error1, forward={'attr2'}, attr1=1, attr3=3
        ):
            raise TestEnumErrors1.Error1(attr1=4, attr2=5)
...
def test_wrap_error() -> None:
    # Test that EnumErrors.wrap_error can catch a specified exception type and
    # set it as a destination attribute.
    class TestErrors(EnumErrors[Exception]):
        Error = 'msg', ('exc', 'arg')

    err = ValueError()
    with enough.raises(TestErrors.Error(exc=err, arg='arg')):
        with TestErrors.Error.wrap_error(ValueError, dest='exc', arg='arg'):
            raise err

    # Won't work with wrong error type.
    with enough.raises(err):
        with TestErrors.Error.wrap_error(TypeError, dest='exc', arg='arg'):
            raise err

    # Multiple types can be used.
    with enough.raises(TestErrors.Error(exc=err, arg='arg')):
        with TestErrors.Error.wrap_error(
            (ValueError, TypeError), dest='exc', arg='arg'
        ):
            raise err

    # dest=None means the exception will not be set for the context.
    with enough.raises(TestErrors.Error(exc=None, arg='arg')):
        with TestErrors.Error.wrap_error(
            ValueError, dest=None, exc=None, arg='arg'
        ):
            raise err
