...
class SecurityErrors(EnumErrors[SecurityError]):
    ...
    CannotAssign = (
        'User {user} does not have sufficient access to modify membership for the following roles: {roles}',
        ('roles', 'user')
    )
    CannotExecute = 'Insufficient access to execute "{path}"', 'state'
    DuplicateRoles = (
        'The following case-insensitive role names are given more than once: {", ".join(duplicated)}', 'duplicated'
    )
    NoSuchRole = 'No role named {name}', 'name'
    UnrecognizedRoles = (
        'The following roles are specified in the values of user_to_roles but not in roles: '
            '{", ".join(role.name for role in unrecognized)}',
        'unrecognized'
    )
    UserCannotExecute = 'User {state.user} does not have sufficient access to execute "{path}"', 'state'
...
class SecurityManager:
    """Determines whether the execution of a command would violate security configurations."""
    def _make_exception(self, state: CommandState) -> SecurityError:
        # Make the exception to raise if there is insufficient access to execute a command.
        return SecurityErrors.CannotExecute(state=state)

    def may_execute(self, state: CommandState) -> bool:
        """Determine whether the command to be executed with the given state would violate security configurations.

        :param state: Current command state.
        :return: :code:`True` if the command may be executed, :code:`False` otherwise.
        """
        return True

    def raise_if_cannot_execute(self, state: CommandState) -> None:
        """Raise an exception if executing the command with the given state would violate security configurations.

        :param state: Current command state.
        :raise SecurityException: If :code:`not self.may_execute(state)`.
        """
...
class RolesSecurityManager(SecurityManager, Generic[T]):
    """Determines whether the execution of a command would violate security configurations by checking if the user has
    a role allowing access.
    """
    ...
    @staticmethod
    def _role_users(user_to_roles: Mapping[T, MutableSet[CommandRole]]) -> defaultdict[CommandRole, set[T]]:
        # From the given mapping of users to their roles, create the inverse mapping from roles to the users that have
        # that role.
    ...
    def _assign_roles(self, roles: Iterable[CommandRole], executor: T, subject: T, is_remove: bool) -> None:
        # Logic for add and remove is almost the same, so put logic in shared function.
        insufficient_access = {role for role in roles if not self.may_assign_role(role, executor)}
        if insufficient_access:
            raise SecurityErrors.CannotAssign(roles=insufficient_access, user=executor)
        if is_remove:
            for role in roles:
    ...
    def __init__(
        ...
        *,
        ...
        :param roles: The roles this security manager will recognize. Must have unique case-insensitive names.
        :param user_to_roles: Optional mapping from user objects to the set of roles that user has to init with.
        :raise SecurityError: If any roles in :code:`roles` have the same name, or if any roles are specified in
            :code:`user_to_roles.values()` but not :code:`roles`.
        """
        name_to_roles = defaultdict(set)
        [name_to_roles[role.name].add(role) for role in roles]
        dup_names = {name for name, roles in name_to_roles.items() if len(roles) > 1}
        if dup_names:
        ...
        unrecognized_roles = {
            role for role_set in self.user_to_roles.values() for role in role_set if role.name not in name_to_roles
        }
        if unrecognized_roles:
            raise SecurityErrors.UnrecognizedRoles(unrecognized=unrecognized_roles)
        self.role_to_users = self._role_users(self.user_to_roles)

    def add_roles(self, roles: Iterable[CommandRole], executor: T, subject: T) -> None:
        ...
        :raise SecurityError: If :code:`executor` does not have sufficient access to assign the given roles.
        """
        self._assign_roles(roles, executor, subject, is_remove=False)

    def may_assign_role(self, role: CommandRole, user: T) -> bool:
        """Determines whether the given user may assign or remove the given role.

        :param role: Role to check.
        :param user: User to check for.
        :return: :code:`True` if :code`executor` may assign the role specified by :code`role_name`, :code`False`
            otherwise.
        """
    ...
    def may_execute(self, state: UserState) -> bool:
        """Determines whether a user has sufficient access to execute a :class:`.Command`. Does not necessarily imply
        that the user has sufficient access to completely carry out the command (i.e., when assigning or removing
        permissions when they do not have access to).

        :param state: The state to use to check access.
        :return: :code:`True` if the user has sufficient access, :code:`False` otherwise.
        """
        return any(r.may_execute(state.query.cmd) for r in self.user_to_roles.setdefault(state.user, set()))

    def remove_roles(self, roles: Iterable[CommandRole], executor: T, subject: T) -> None:
