...
class ParseError(KeywordCommandsError):
    """Exception to raise to indicate that a foreseen parsing error occurred in
    a parse function.
    """


class ChainedParseError(ParseError, WrappedException):
    """Exception to raise when a :class:`.ParsingException` results from another
    exception.
    """
...
class ParserInitErrors(EnumErrors[KeywordCommandsError]):
    ...
    BadKWArgs = (
        'The given function has keyword-only arguments without defaults.', 'fn'
    )
    BadNumArgs = (
        'The given functions takes {num_args} positional arguments when either '
            '1 or 2 are expected.',
        'fn'
    )
...
class Parser(Generic[V]):
    ...
    #: String containing a description of how arguments parsed by this parser
    #: should be formatted.
    expected_format: str
    ...
    @staticmethod
    def ensure_state_arg(
        fn: ParseFunction[V]
    ) -> Callable[[str, CommandState], V]:
        """If ``fn`` takes a single argument, convert it to a function
        containing two arguments. The second argument is the unused state. If
        ``fn`` already takes two arguments, it is returned instead.
        
        :param fn: Function to convert.
        :return: Parse function taking two arguments.
        :raise ParserInitError: If ``fn`` takes a number of positional
            arguments differing from 1 or 2, or if it has keyword-only arguments
            without defaults.
        """
        try:
            ...
            if 'no signature found for builtin type' in traceback.format_exc():
                # Assume the built-in type takes a single argument as a means of
                # conversion to it.
                return lambda arg, state: fn(arg)
            raise
        # Note that spec.kwonlydefaults is None if no arguments are
        # keyword-only.
    ...
    def __init__(self, fn: ParseFunction[V], expected_format: str) -> None:
        ...
        :param fn: Parse function to use. The first argument it takes should be
            the string argument to parse, while the second argument is the
            current state. Optionally, the function may take only one string
            argument and omit the state argument.
        :param expected_format: String which serves as a user message for how
            arguments should be formatted.
        :raise ParserInitError: If ``fn`` takes a number of positional arguments
            differing from 1 or 2, or if it has keyword-only arguments without
            defaults.
        """
...
Parser.DEFAULT = Parser(enough.identity, 'Anything')
...
def parser(expected_format: str) -> Callable[[ParseFunction[V]], Parser[V]]:
    ...
    :class:`.Parser`. That function should take either a single string argument
    or a string argument and a :class:`.CommandState` argument.

    :param expected_format: String containing a description of how arguments
        parsed by this parser should be formatted.
