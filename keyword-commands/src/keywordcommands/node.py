...
class _CommandNode(ABC):
    # Base class of Command and CommandGroup, which can benefit from shared implementation of commands and find.
    def commands(self) -> list[tuple[list[str], Command]]:
        """Gives a sequence of tuples each containing the path to a command reachable from this node as its first
        element and the :class:`Command` itself as the second element.

        :return: Sequence of :code:`(path, command)` for commands reachable from this node.
        """
        from keywordcommands.command import Command
        return [(path, cmd_or_group) for path, cmd_or_group in self.paths() if isinstance(cmd_or_group, Command)]
    ...
    def find(self, path: Sequence[str]) -> Command | CommandGroup | None:
        ...
        :return: The found node, or :code:`None` if no node could be found.
        """
        if not path:
    ...
    def paths(self) -> list[tuple[list[str], Command | CommandGroup]]:
        ...
        :return: (path, command or group) for each command or group accessible from this node.
        """
        result = [([], self)]
        for edge, node in self.tree().items():
            result += [(br.concat([edge], path), cmd_or_group) for path, cmd_or_group in node.paths()]
        # noinspection PyTypeChecker
        return result
    ...
    def tree(self) -> Mapping[str, Command | CommandGroup]:
        ...
        vertices, and :class:`Commands <.Command>` are the leaf vertices.
