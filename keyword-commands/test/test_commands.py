...
from keywordcommands import CommandRole, Example, Parser, RolesSecurityManager, commands
from keywordcommands.exceptions import ChainedExecutionError, CommandErrors, SecurityErrors
...
def test_assign_roles_cmd(
    ...
    # Test commands._assign_roles_cmd, a helper method containing the core logic for add_roles_cmd and remove_roles_cmd.
    ...
    assert cmd.arg('users').parser.expected_format == 'Comma-separated list of users_format'
    ...
    # Make sure a SecurityErrors.CANNOT_ASSIGN leads to ChainedExecutionException being raised.
    # Need path and text to be set for this since the exception uses it to format a message.
    ...
    with br.raises(CommandErrors.ExecutionError(error=ChainedExecutionError(error), query=user_state.query)):
        cmd(user_state, parsed)


def test_add_roles_cmd(void_examples: list[Example]) -> None:
    # Just test that _assign_roles_cmd is properly called.
    with mock.patch('keywordcommands.commands._assign_roles_cmd') as mock_helper:
        assert (
            commands.add_roles_cmd(user_parser=Parser.DEFAULT, examples=void_examples, users_format='users_format')
            == mock_helper.return_value
        )
        kwargs = mock_helper.call_args.kwargs
        # Checking default messages is frivolous and error-prone, check other args instead.
...
def test_default_user_role_formatter(role_empty: CommandRole, role1: CommandRole, role2: CommandRole) -> None:
    # Test commands.default_user_role_formatter, which lists the roles for each user on a separate line, or vice-versa.
...
def test_remove_roles_cmd(void_examples: list[Example]) -> None:
    ...
    with mock.patch('keywordcommands.commands._assign_roles_cmd') as mock_helper:
        assert (
            commands.remove_roles_cmd(user_parser=Parser.DEFAULT, examples=void_examples, users_format='users_format')
            == mock_helper.return_value
        )
        kwargs = mock_helper.call_args.kwargs
        # Checking default messages is frivolous and error-prone, check other args instead.
...
def test_show_role_users_cmd(
    ...
    # Test that commands.show_role_users_cmd creates a command that prints the users assigned with the specified roles.
    mock_formatter = Mock(spec=Callable)
    cmd = commands.show_role_users_cmd(examples=void_examples, formatter=mock_formatter)
    ...
    cmd(user_state, parsed)
    mock_formatter.assert_called_with({role_empty: {'user3'}, role1: {'user2', 'user3'}, role2: {'user3'}})
...
def test_show_user_roles_cmd(
    ...
    # Test that commands.show_user_roles_cmd creates a command that prints the roles assign to the specified users.
    mock_formatter = Mock(spec=Callable)
    cmd = commands.show_user_roles_cmd(
        examples=void_examples, formatter=mock_formatter, users_format='users_format', user_parser=Parser.DEFAULT
    )
    assert cmd.arg('users').parser.expected_format == 'Comma-separated list of users_format'
    ...
    cmd(user_state, parsed)
    mock_formatter.assert_called_with({'user1': set(), 'user2': {role1}, 'user3': {role_empty, role1, role2}})
