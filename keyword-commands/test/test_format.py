...
from keywordcommands import (
    Arg, Command, CommandGroup, DefaultQueryFormatFns, DefaultQueryFormatter, Example, Parser, QueryInfo, QueryResult
)
from keywordcommands.exceptions import CommandErrors, FormatErrors


def test_compose_formats() -> None:
    # Test keywordcommands.format.compose_fmts, which should take a mapping from format fields to format strings, where
    # each format field in the string is replaces by the corresponding expanded format string.
    assert keywordcommands.format.compose_fmts(
        {'field1': 'fmt1', 'field2': 'fmt2 {field1}', 'field3': '{field2}{field1}'}
    ) == {'field1': 'fmt1', 'field2': 'fmt2 fmt1', 'field3': 'fmt2 fmt1fmt1'}

    # ignore kwarg allows some format fields to be left untouched.
    assert keywordcommands.format.compose_fmts(
        {'field1': 'val is {val1}', 'field2': '{field1} {val2}', 'field3': '{field2} {val3}'},
        ignore={'val1', 'val2', 'val3'}
    ) == {'field1': 'val is {val1}', 'field2': 'val is {val1} {val2}', 'field3': 'val is {val1} {val2} {val3}'}

    # Try when a field is both ignored and included.

    with br.raises(FormatErrors.ComposeFieldNameCollision(colliding={'val1', 'val2'})):
        keywordcommands.format.compose_fmts(
            {'field1': '{val1}', 'field2': '{val2}', 'field3': '{val3}', 'val1': 'asdf', 'val2': 'fdsa'},
            ignore={'val1', 'val2', 'val3'}
        )

    # Try when there are missing unignored fields.
    with br.raises(FormatErrors.ComposeMissingFields(missing={'val2', 'val3'})):
        keywordcommands.format.compose_fmts(
    ...
    with pytest.raises(FormatErrors.CircularDependency) as exc_info:
        keywordcommands.format.compose_fmts({'field1': '{field2}', 'field2': '{field3}', 'field3': '{field1}'})
...
def test_default_query_format_fns() -> None:
    ...
    example1 = Example('does something.', OrderedDict([('arg1', 'val1'), ('arg2', '5')]))
    example2 = Example('does something else.', OrderedDict([('arg1', 'VAL1'), ('arg2', '6')]))
    ...
    # Test formatting that does not depend or specifically relate to how node or error is set.
    assert fns['app'](query) == 'App'
    assert fns['path'](query) == 'edge3 edge1'
    assert format_fns.fmt_coll(['first', 'second', 'third']) == 'first, second, third'

    # Test formatting command information.
    query.node = cmd1
    assert fns['cmd_desc'](query) == 'command 1.'  # Description should be uncapitalized.
    assert format_fns.fmt_cmd_short(cmd1, ['edge3', 'edge1']) == 'edge3 edge1: Command 1.'
    ...
    assert format_fns.fmt_example(example1, query.path) == expected_example_lines[0]
    assert format_fns.fmt_examples(cmd1.examples, query.path) == '\n'.join(expected_example_lines)
    ...
    query.path = ['edge3']
    assert format_fns.fmt_cmds(group, ['edge3']) == '\n'.join(expected_cmd_lines)
    assert fns['cmds'](query) == '\n'.join(expected_cmd_lines)
    assert fns['root_cmds'](query) == '\n'.join(expected_root_lines)

    # Test error formatting.
    query.error = CommandErrors.DuplicateArgs(duplicated=['1', '2', '3'], query=query)
    assert fns['duplicated_args'](query) == '1, 2, 3'
    assert fns['error'](query) == str(query.error)

    query.error = CommandErrors.ParseError(arg=arg1, value='val1', error=ValueError(), query=query)
    assert fns['err_actual'](query) == 'val1'
    assert fns['err_arg'](query) == 'arg1'
    assert fns['err_expected'](query) == 'Anything'

    query.error = CommandErrors.MissingRequiredArgs(missing=['arg1', 'arg2'], query=query)
    assert fns['missing_args'](query) == 'arg1, arg2'

    query.error = CommandErrors.UnrecognizedArgs(unrecognized=['arg5', 'arg6'], query=query)
    ...
    with mock.patch('traceback.format_exception') as mock_fmt_exc:
        ...
        assert format_fns.fmt_unexpected(query.error) == 'An unexpected failure occurred'
...
def test_default_query_formatter() -> None:
    ...
    with br.raises(
        FormatErrors.MissingResultFields(missing={QueryResult.GENERAL_HELP, CommandErrors.UnexpectedError})
    ):
        DefaultQueryFormatter(
            field_to_fmt=field_to_fmt, fmt_fns=format_fns, result_to_field=result_to_field_with_missing
        )

    # Missing a format field that appears in result_to_field should result in an exception.
    ...
    with br.raises(FormatErrors.FormatterMissingFields1(missing={'field3', 'field4'})):
        DefaultQueryFormatter(
            field_to_fmt=field_to_fmt, fmt_fns=format_fns, result_to_field=result_to_field_with_unrecognized
        )
    ...
    formatter = DefaultQueryFormatter(field_to_fmt=field_to_fmt, fmt_fns=format_fns, result_to_field=result_to_field)
    assert formatter.field_to_format == {'field1': 'value1', 'field2': 'value1 value2 {path}'}
    ...
    query.result = None
    with br.raises(FormatErrors.NoResult(query=query)):
