...
def test_may_execute(
    ...
    # First, establish that test_role.may_execute(cmd) returns True whenever cmd
    # in test_role.cmds.
    ...
    # Now there is no need to use may_execute for each command we want to verify
    # may execute. We can instead just verify the CommandRole.cmds is exactly
    # the set of commands that are expected to executable.
    ...
    assert CommandRole(
        '', cmds=[cmd1, cmd3], groups=[group1]
    ).cmds == {cmd1, cmd2, cmd3}
    assert CommandRole(
        '', cmds=[cmd_no_args], groups=[group1, group3]
    ).cmds == {cmd_no_args, cmd1, cmd2, cmd3, cmd4}
    assert CommandRole(
        '', bases=[CommandRole('', cmds=[cmd1, cmd2])]
    ).cmds == {cmd1, cmd2}
    assert CommandRole(
        '', bases=[CommandRole('', groups=[group1, group2])]
    ).cmds == {cmd_no_args, cmd1, cmd2}
    assert CommandRole(
        '',
        bases=[
            CommandRole('', cmds=[cmd1, cmd2]),
            CommandRole('', groups=[group3])
        ]
    ).cmds == {cmd1, cmd2, cmd3, cmd4}
    assert CommandRole(
        '',
        cmds=[cmd1, cmd2],
        bases=[CommandRole('', groups=[group3])]
    ).cmds == {cmd1, cmd2, cmd3, cmd4}
    assert CommandRole(
        '',
        cmds=[cmd_no_args],
...
def test_may_assign(
    ...
    role = CommandRole(
        '', assignable_roles=[role_empty, role2], may_assign_self=True
    )
    ...
    # While base roles are not necessarily assignable, a role will inherit the
    # assignable roles of its bases.
