...
def test_check_for_dupes() -> None:
    # Test CommandGroup._check_for_dupes, which should check a given mapping for
    # any case-insensitive duplicate keys.

    # Should succeed.
    CommandGroup._check_for_dupes({
        'a': Mock(spec=Command | CommandGroup),
        'b': Mock(spec=Command | CommandGroup)
    })

    # Should fail.
    with enough.raises(GroupInitErrors.DuplicateEdges(duplicated={'a'})):
        CommandGroup._check_for_dupes({
            'a': Mock(spec=Command | CommandGroup),
            'A': Mock(spec=Command | CommandGroup)
        })
...
def test_malformed_edges() -> None:
    # Test CommandGroup._malformed_edges, which should indicate which strings
    # from a collection contain invalid characters.
    CommandGroup._check_malformed_edges({'asdf', 'fdsa'})
    with enough.raises(GroupInitErrors.MalformedEdges(malformed={'$asdf'})):
        CommandGroup._check_malformed_edges({'$asdf', 'fdsa'})
...
def test_group(cmd_no_args: Command, cmd1: Command, cmd2: Command) -> None:
    ...
    with mock.patch.multiple(
        CommandGroup,
        _check_for_dupes=mock.DEFAULT,
        _check_malformed_edges=mock.DEFAULT
    ) as mocks:
    ...
    # group.find should be able to find the commands, or itself with an empty =
    # path.
    ...
    # Test group.commands(), which should give a list of (path, cmd) pairs in =
    # alphabetical order.
    ...
    # commands() should find all commands, even those which are not direct =
    # edges.
