...
from keywordcommands import Arg, Command, CommandGroup, CommandState, Example, Parser, command
from keywordcommands.exceptions import CommandErrors, CommandInitErrors, ExecutionError, ParseError

from conftest import cmd1_fn


def test_check_examples(cmd1: Command) -> None:
    # Test Command._check_examples, which should make sure the arguments of all examples not marked as "unchecked" can
    # be parsed without error.
    # The fixture examples, which should not generate errors, have already been tested by this point, or else the
    # command could not be constructed.
    args = [Arg('Argument 1', 'arg1'), Arg('Argument 2', 'arg2'), Arg('Argument 3', 'arg3')]
    cmd1.examples = [Example('Example 3', {'arg1': 'val1', 'arg2': 'val2', 'arg3': 'val3'}, unchecked=False)]

    with br.raises(CommandInitErrors.BadExample(
        args=args,
        error=CommandErrors.UnrecognizedArgs(unrecognized={'arg3'}, query=Command._void_state().query),
...
def test_check_signature(cmd1: Command) -> None:
    # Test Command._check_signature, which should raise an exception if the sole positional argument of the supplied
    # command is not 'state', or if there is a mismatch between the names of the arguments passed to Command.__init__
    # and the keyword arguments for the function.

    # First, change which args are recognized by cmd and then expect an exception.
    old_spec = copy.copy(cmd1.args)
    args = [Arg('Arg 1', 'arg1')]
    cmd1.args = {'arg1': Arg('Arg 1', 'arg1')}
    with br.raises(CommandInitErrors.ExtraArgs(args=args, extra=['arg2'], fn=cmd1._fn)):
        cmd1._check_signature(args)

    # Now add an additional arg and expect an exception.
    cmd1.args = {**old_spec, 'arg3': Arg('Arg 3', 'arg3')}
    with br.raises(CommandInitErrors.MissingArgs(args=args, missing=['arg3'], fn=cmd1._fn)):
        cmd1._check_signature(args)
    cmd1.args = old_spec

    # Now use a function with argument names containing uppercased characters and expect an exception.
    # noinspection PyUnusedLocal, PyPep8Naming
    def cap_fn(state: CommandState, *, arg1: str, ArG2: str) -> None: pass

    cmd1._fn = cap_fn
    with br.raises(CommandInitErrors.UppercasedArgs(upper=['ArG2'], fn=cap_fn)):
        cmd1._check_signature(args)
    ...
    with br.raises(CommandInitErrors.VariadicArg(arg='arg1', fn=var_args_fn)):
        cmd1._check_signature(args)

    # Keyword var args.
    # noinspection PyUnusedLocal
    def kw_var_args_fn(state: CommandState, *, arg1: str, **arg2: str) -> None: pass

    cmd1._fn = kw_var_args_fn
    with br.raises(CommandInitErrors.VariadicArg(arg='arg2', fn=kw_var_args_fn)):
        cmd1._check_signature(args)

    # Both positional and keyword var args.
    # noinspection PyUnusedLocal
    def pos_and_kw_var_args(state: CommandState, *arg1: str, **arg2: str) -> None: pass

    cmd1._fn = pos_and_kw_var_args
    # arg1 is the first variadic argument to appear.
    with br.raises(CommandInitErrors.VariadicArg(arg='arg1', fn=pos_and_kw_var_args)):
        cmd1._check_signature(args)

    # Now use a function with underscores corresponding to an arg with hyphens and check that that works.
    # noinspection PyUnusedLocal
    def under_fn(state: CommandState, *, arg1: str, arg_2: str, __arg3__: str) -> None: pass

    args = [Arg('Arg 1', 'arg1'), Arg('Arg 2', 'arg-2'), Arg('Arg 3', '--arg3--')]
    Command('Command', under_fn, args=args)

    # Now make a function that has positional arguments differing from just 'state'.
    ...
    cmd1._fn = no_state
    with br.raises(CommandInitErrors.NonStatePosArg(fn=no_state)):
        cmd1._check_signature(args)

    # Second: An extra positional argument.
    # noinspection PyUnusedLocal
    def extra_pos(state: CommandState, crate: CommandState, *, arg1: str, arg2: str) -> None: pass

    cmd1._fn = extra_pos
    with br.raises(CommandInitErrors.NonStatePosArg(fn=extra_pos)):
        cmd1._check_signature(args)
    ...
    cmd1._fn = no_pos
    with br.raises(CommandInitErrors.NonStatePosArg(fn=no_pos)):
        cmd1._check_signature(args)
...
def test_init(arg1: Arg, arg2: Arg, examples: list[Example]) -> None:
    ...
    with mock.patch.multiple(
        Command, autospec=True, _check_examples=mock.DEFAULT, _check_signature=mock.DEFAULT
    ) as mocks:
        cmd = Command('Command description', cmd1_fn, args=[arg1, arg2], examples=examples)
...
def test_arg_properties(arg1: Arg, arg2: Arg, cmd1: Command) -> None:
    # Test that Command.optional_args and Command.required_args correctly return the optional and required arguments
    # respectively.
...
def test_parse(arg1: Arg, arg2: Arg, cmd1: Command) -> None:
    ...
    with br.raises(CommandErrors.MissingRequiredArgs(missing={'arg1'}, query=Command._void_state().query)):
        cmd1.parse(None, {'arg2': 'value2'})

    # Now with extra args.
    with br.raises(
        CommandErrors.UnrecognizedArgs(unrecognized={'arg3', 'arg4'}, query=cmd1._void_state().query)
    ):
        cmd1.parse(None, {'arg1': 'value1', 'arg3': 'value3', 'arg4': 'value4'})

    # Try to succeed.
    assert cmd1.parse(None, {'arg1': 'value1'}) == {'arg1': 'value1'}
    assert cmd1.parse(None, {'arg1': 'value1', 'arg2': 'value2'}) == {'arg1': 'value1', 'arg2': 'value2'}
    ...
    # Succeed first.
    assert cmd1.parse(None, {'arg1': '3', 'arg2': '5'}) == {'arg1': 3, 'arg2': 5}

    # Now fail.
    with br.raises(CommandErrors.ParseError(
        arg=arg1, value='string', error=parse_error, query=cmd1._void_state().query)
    ):
...
def test_call(cmd1: Command) -> None:
    # Test Command.__call__, which should attempt to parse the given args and then pass the parsed args to the supplied
    # function.
    mock_fn = Mock()
    ...
    # Try when a different exception is raised, which will not be wrapped in CommandFailedException.
...
def test_decorator(arg1: Arg, arg2: Arg, examples: list[Example]) -> None:
    ...
    def command_fn(state: CommandState, *, arg1: str, arg2: str = 'asdf') -> None: pass
