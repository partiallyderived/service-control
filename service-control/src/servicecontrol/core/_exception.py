...
class ServiceControlError(Exception):
    ...
    @property
    def _enough(self) -> ModuleType:
        # Gives the enough module so that it may be used in f-strings.
        return enough
    ...
    def _fmt_error_map(
        self, key_fn: Callable[[T], object] = enough.identity
    ) -> str:
        # Gives a string representation of a mapping from objects to their
        # associated exceptions.
        return '\n\n'.join(
            f'{key_fn(k)}: {"".join(traceback.format_exception(e))}'
