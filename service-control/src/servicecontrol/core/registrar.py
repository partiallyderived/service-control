...
class RegistrationErrors(EnumErrors[ServiceControlError]):
    ...
    ImplicitExists = (
        'Failed to register implicit service {service.cls_name()}: the following objects are already registered as '
            'exports for an implicit service: {", ".join(exports)}',
        ('exports', 'service')
    )
    NotAService = '{_br.fqln(cls)} is not a subclass of servicecontrol.core.Service.', 'cls', TypeError
    RegistrationExists = (
        'Failed to register service {attempted.cls_name()} with the name "{name}": The service class '
            '{registered.cls_name()} is already registered with that name.',
        ('name', 'attempted', 'registered')
    )


def find(name: str) -> type[Service] | None:
    """Finds the service which is registered with the given name, case-insensitive, or else return :code:`None`.
...
def find_implicit(name: str) -> type[Service] | None:
    """Finds an implicitly-available service that exports an object with the given name.
...
def recursively_register(package: ModuleType) -> None:
    """Recursively registers all service classes which can be found in the given package or any of its subpackages.
    and subpackages, or any of their submodules and subpackages, etc.

    :param package: Name of the package to recurse over.
    """
    for _, mod_name, is_pkg in pkgutil.iter_modules(package.__path__, prefix=f'{package.__name__}.'):
...
def register(name: str, service_type: type[Service]) -> None:
    ...
    :raise ServiceControlError: If any of the following are true:
        * The service to be registered is implicit but the names of at least one of its exports is the same as one for a
          previously registered implicit service.
        * The service to be registered is implicit but the names of at least one of its exports is the same as one for a
          previously registered implicit service.
        * A service is already registered with :code:`name`.
    """
    ...
    if existing:
        raise RegistrationErrors.RegistrationExists(attempted=service_type, name=name, registered=existing)
    if service_type.implicit():
        existing_implicits = {x for x in service_type.export_names() if x in _IMPLICIT_EXPORTS}
        if existing_implicits:
            raise RegistrationErrors.ImplicitExists(exports=existing_implicits, service=service_type)
...
def register_all_in_module(module: ModuleType) -> None:
    """Registers all services in the given module which have overridden :meth:`.Service.default_name`.

    :param module: Module to register services for.
    """
    for cls in br.module_members(
        module, lambda m: isinstance(m, type) and issubclass(m, Service) and m.default_name() is not None
    ):
...
def register_defaults() -> None:
    """If it has not been done already, recursively registers all services contained in the servicecontrol package."""
