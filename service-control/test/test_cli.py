...
def mock_parsed(config: JSONType) -> Mock:
    ...
    # As config is modified when loading service specs, this should be a copy of
    # the config.
...
def test_controller(mock_parsed: Mock) -> None:
    # Test that _controller correctly raises a CLIError is the controller fails
    # to initialize.
    with mock.patch.object(
        cli.Controller, '__init__', autospec=True
    ) as mock_init:
        err = ControllerErrors.NoSuchService(name='asdf')
        mock_init.side_effect = err
        with enough.raises(CLIErrors.ControllerInit(error=err)):
    ...
    with NamedTemporaryFile('w') as config_file:
        ...
        with mock.patch.multiple(
            ...
            mock_start.assert_called_with(
                Namespace(command='start', config=config)
            )

            # Without service, job should fail.
            with enough.raises(CLIErrors.ArgParse()):
                cli.main(['job', config_file.name])

            cli.main(['job', config_file.name, 'SomeService'])
            mock_job.assert_called_with(Namespace(
                command='job', config=config, service='SomeService', args=[]
            ))

            # Specify a non-existent config file.
            not_a_file = 'This is surely not a file.'
            with enough.raises(CLIErrors.NotAFile(name=not_a_file)):
            ...
            cli.main([
                'purge',
                config_file.name,
                Service1.default_name(),
                Service2.default_name()
            ])
            mock_purge.assert_called_with(Namespace(
                command='purge',
                config=config,
                services=[Service1.default_name(), Service2.default_name()]
            ))
            with enough.raises(CLIErrors.ArgParse()):
            ...
            mock_purge.assert_called_with(Namespace(
                command='purge',
                config=config,
                services=[Service1.default_name()]
            ))
...
def test_job(config: JSONType, mock_parsed: Mock) -> None:
    # Test that the logic for running a job from command line arguments is
    # correct.
    ...
    with enough.raises(CLIErrors.JobNoSuchService(name='does-not-exist')):
        cli.job(mock_parsed)


def test_purge(config: JSONType, mock_parsed: Mock) -> None:
    # Test that the logic for purging specified controller services from parsed
    ...
    with mock.patch.object(Service1, 'purge', autospec=True) as mock_purge1:
        with mock.patch.object(Service2, 'purge', autospec=True) as mock_purge2:
            mock_parsed.services = [
                Service1.default_name(), Service2.default_name()
            ]
            ...
            with enough.raises(CLIErrors.PurgeNoSuchService(
                not_found={'does-not-exist'}
            )):
                cli.purge(mock_parsed)
            mock_parsed.config = copy.deepcopy(config)

            # Even if both purges raise an exception, both purges should still
            # end up being called.
            mock_parsed.services = [
                Service1.default_name(), Service2.default_name()
            ]
            ...
            for service, error in errors.items():
                if isinstance(service, Service1):
                    ...
                    expected = err2
                assert error == ControllerErrors.PurgeFailed(
                    error=expected, service=service
                )
            mock_purge1.assert_called()
            mock_purge2.assert_called()
...
def test_start(mock_parsed: Mock) -> None:
    # Test that the logic for starting a controller from parsed command line
    # arguments is correct.
    with mock.patch('signal.signal', autospec=True) as mock_signal:
        with mock.patch.multiple(
            Service1, start=mock.DEFAULT, stop=mock.DEFAULT
        ) as mocks1:
            with mock.patch.multiple(
                Service2, start=mock.DEFAULT, stop=mock.DEFAULT
            ) as mocks2:
                ...
                # Wait a second, then check that the thread is alive and that
                # start methods are called while stop methods have not been
                # called yet.
                time.sleep(1)
                ...
                # Allow half a second (very generous since there is not stop
                # logic for the services) for thread to terminate, and then
                # check that stop methods were called.
