...
class SomeService(Service):
    EXPORTS: Final[frozenset[str]] = frozenset({'export1', 'export2', 'export3'})
...
def test_check_unrecognized(spec: ServiceSpec) -> None:
    # Check that ServiceSpec._check_unrecognized correctly raises an exception when unrecognized overrides for
    # dependencies or exports are specified.

    # Check passes when the second argument (overrides) is a subset of the first argument (recognized).
    ...
    spec._check_unrecognized({'arg1', 'arg2'}, set(), ServiceSpecErrors.NoSuchDependency)
    spec._check_unrecognized({'arg1', 'arg2'}, {'arg1'}, ServiceSpecErrors.NoSuchExport)
    spec._check_unrecognized({'arg1', 'arg2'}, {'arg1', 'arg2'}, ServiceSpecErrors.NoSuchDependency)

    # Otherwise, check should fail, raising the given exception and formatting the exception message using the given
    # string.
    with br.raises(ServiceSpecErrors.NoSuchExport(config=spec.config, names={'arg1'}, service=spec.cls)):
        spec._check_unrecognized(set(), {'arg1'}, ServiceSpecErrors.NoSuchExport)

    with br.raises(ServiceSpecErrors.NoSuchDependency(
        config=spec.config, names={'arg2', 'arg3'}, service=spec.cls)
    ):
        spec._check_unrecognized({'arg1'}, {'arg1', 'arg2', 'arg3'}, ServiceSpecErrors.NoSuchDependency)


def test_resolve_class_and_name(spec: ServiceSpec) -> None:
    # Test that ServiceSpec._resolve_class_and_name correctly infers a service's name and class from configured values
    ...
    with br.raises(ServiceSpecErrors.MissingName(config=spec.config, service=NoNameService)):
    ...
    assert isinstance(exc_info.value.error, BRError)

    # Only name specified, no registration.
    with br.raises(ServiceSpecErrors.NoSuchService(name='TestName')):
    ...
    with br.raises(ServiceSpecErrors.MissingClass(config=spec.config)):
        spec._resolve_class_and_name(None, None)


def test_validate(spec: ServiceSpec) -> None:
    # Test the ServiceSpec._validate correctly calls both Service.check_init() and Service.validate().

    with mock.patch.multiple(spec.cls, check_init=mock.DEFAULT, validate=mock.DEFAULT, autospec=True) as mocks:
        spec._validate({})
...
def test_export_names(spec: ServiceSpec) -> None:
    ...
    assert spec.export_names() == {'export1', 'export2_override', 'export3_override'}


def test_call(spec: ServiceSpec) -> None:
    # It is the responsibility of the Controller to check that spec is called with the correct arguments.
...
def test_init(service_config: JSONType) -> None:
    ...
    # Test that a configuration for a class which is not a service results in a NotAServiceError.
    with br.raises(ServiceSpecErrors.NotAService(cls=Thread)):
    ...
    with mock.patch.multiple(
        ServiceSpec, _check_unrecognized=mock.DEFAULT, _validate=mock.DEFAULT, autospec=True
    ) as mocks:
        spec = ServiceSpec(service_config)

        mocks['_check_unrecognized'].assert_any_call(
            spec, {'dep1', 'dep2', 'dep3'}, {'dep1'}, ServiceSpecErrors.NoSuchDependency
        )
        mocks['_check_unrecognized'].assert_any_call(
            spec, {'export1', 'export2', 'export3'}, {'export2', 'export3'}, ServiceSpecErrors.NoSuchExport
        )
