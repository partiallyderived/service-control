...
class EAContactsSyncService(Service):
    """Service which periodically updates EveryAction contact data for :code:`EAContactsService`."""
    ...
    _CONTACT_FIELDS: Final[set[str]] = {
        'ChangeTypeID', 'DoNotCall', 'DoNotEmail', 'FirstName', 'LastName', 'PersonalEmail', 'Phone', 'VanID'
    }

    # Handlers setting a ContactUpdate's state when values are being deleted.
    _DELETE_HANDLERS: Final[dict[str, Callable[[ContactUpdate, object], None]]] = {
    ...
    # Handlers for setting a ContactUpdate's state when values are being created or updated.
    _UPDATE_HANDLERS: Final[dict[str, Callable[[ContactUpdate, object], None]]] = {
    ...
    SCHEMA: Final[JSONType] = {
        ...
        'properties': {
            'period': {
                'description': f'Period in seconds between two contact syncs ({DEFAULT_PERIOD} by default).',
                'type': 'integer'
            },
            'start': {
                'description': 'ISO-formatted date to start contacts sync. Required for installation.',
                'type': 'string'
            }
        },
    ...
    # The list of ChangedEntityFields to cache for changed entity export job requests on the 'ContactsActivistCode'
    # resource.
    _code_fields: list[ChangedEntityField]

    # The list of ChangedEntityFields to cache for changed entity export job requests on the 'Contacts' resource.
    ...
    def _code_update(self, changes: MutableMapping[str, ChangedEntityField.ValueType]) -> ContactUpdate | None:
        ...
        change_type = self._change_types['ContactsActivistCodes'][change_type_id].name
    ...
    def _contact_update(self, changes: MutableMapping[str, ChangedEntityField.ValueType]) -> ContactUpdate | None:
    ...
    def _schedule_update(self) -> None:
        # Update the EveryAction contacts database and then schedule it to be updated again after the configured period.
        self._update()
        self._scheduler.enter(self._period, 1, self._schedule_update)

    def _update(self) -> None:
        # Update the EveryAction contacts database by incorporating changes made since the last update.
    ...
    def _update_codes(self, start: str, end: str, updates: MutableSequence[ContactUpdate]) -> None:
    ...
    def _update_contacts(self, start: str, end: str, updates: MutableSequence[ContactUpdate]) -> None:
    ...
    def __init__(
        ...
        entity_fields_cache: CacheProto[str, CacheProto[str, ChangedEntityField]],
        scheduler: sched.scheduler
    ) -> None:
        """Initializes this service with the given config and dependencies.

        :param config: Config to initialize with.
        :param change_types_cache: Cached map of resource names to maps of IDs/names of change types to ChangeType
            objects.
        ...
        :param entity_fields_cache: Cached map of resource names to maps of names of changed entity fields to
            ChangedEntityField objects.
    ...
    def install(self) -> None:
        """Installs this service by persisting data about when the last sync occurred.
        ...
        if not self._start_time:
            raise ValueError(
                'The configuration key "start" must be specified on installation to the time from which to start '
                'syncing.'
            )
    ...
    def installed(self) -> bool:
        """Determines if this service needs to be installed by detecting if the name of this service is in the data
        mapping.
        """
    ...
    def start(self) -> None:
        ...
        self._code_fields = [self._fields['ContactsActivistCodes'][x] for x in self._CODE_FIELDS]
        self._contact_fields = [self._fields['Contacts'][x] for x in self._CONTACT_FIELDS]
        # Start updating. Entered into the scheduler this way so that updating happens (almost) immediately, but other
        # services need not wait on this service to start.
