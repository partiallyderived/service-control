...
def test_contact_update() -> None:
    ...
    assert update._update() == {"$set": {"first": "Alice", "last": "Simpson", "do_not_email": True}}

    # Empty strings should unset.
    update.first = ""
    update.last = ""
    assert update._update() == {"$set": {"do_not_email": True}, "$unset": {"first": "", "last": ""}}
...
def test_from_person() -> None:
    # Test that ContactUpdate.from_person correct transforms a Person object into a MongoDB contact document.
...
def test_code_name() -> None:
    # Test that _code_name correctly strips the committee off a header for an activist code.

    assert EAContactsService._code_name("Some_Activist_(Atlanta)") == "Some_Activist"


def test_extract_suppression() -> None:
    # That that _extract_suppression correctly converts 0 and 1 to False and True respectively.
...
def test_find_query() -> None:
    # Test that _find_query correctly constructs a query with the appropriate arguments.
...
def test_line_to_doc() -> None:
    # Test various cases of the usage of _line_to_doc to check that a single line is correctly parsed.
    ...
    assert EAContactsService._line_to_doc(
        "3\tfake@fake.com\tx\t1234567890\tx", cols_and_extractors, cols_and_code_ids
    ) == {
        "int": 3,
    ...
    # Try with some empty fields. Note that codes, phones, and emails should always be set.
    assert EAContactsService._line_to_doc("\tfake@fake.com\tx\t\t", cols_and_extractors, cols_and_code_ids) == {
        "emails": ["fake@fake.com"],
        "phones": [],
        "codes": [9]
    }
    ...
    assert EAContactsService._line_to_doc("\t\t\t\t", cols_and_extractors, cols_and_code_ids) == {
        "codes": [],
        "emails": [],
        "phones": []
    }
...
def test_wrap_in_list() -> None:
    # Test that _wrap_in_list correctly wraps non-empty strings in lists or else returns None.
...
def alice_update(alice_doc: JSONType) -> ContactUpdate:
    # A ContactUpdate that, when used to create a document, results in alice_doc.
...
def bob_update(bob_doc: JSONType) -> ContactUpdate:
    ...
    return ContactUpdate(
        2, first=bob_doc["first"], last=bob_doc["last"], add_emails=bob_doc["emails"], add_codes=bob_doc["codes"]
    )
...
def test_init(mock_code_cache: Mock, mongo_mock: MongoClient) -> None:
    ...
    service = EAContactsService({}, activist_codes_cache=mock_code_cache, mongo=mongo_mock)
...
@pytest.fixture
def service(mock_code_cache: Mock, mongo_mock: MongoClient) -> EAContactsService:
...
def test_init_indexes(service: EAContactsService) -> None:
    ...
    index_info = sorted(list(service.collection.index_information().values()), key=lambda x: x["key"][0][0])
    ...
    assert index_info == [
        ...
        {"key": [("first", pymongo.ASCENDING), ("last", pymongo.ASCENDING)], "sparse": True},
...
def test_load_contacts(service: EAContactsService) -> None:
    # Test that load contacts correctly processes an exported contacts file and delegates logic to helper functions.
    with NamedTemporaryFile("w") as f:
        header = "VANID\tFirst\tLast\tActivist_Code1_(Committee)\tActivist_Code2_(Committee)"
        ...
        with mock.patch.multiple(
            service, autospec=True, _line_to_doc=mock.DEFAULT, _process_header=mock.DEFAULT
        ) as mocks:
            ...
            mock_process_header.return_value = (mock_cols_and_extractors, mock_cols_and_code_ids)
            service._load_contacts(f.name)
            mock_process_header.assert_called_with(header)
            assert mock_line_to_doc.call_args_list == [
                mock.call(line1, mock_cols_and_extractors, mock_cols_and_code_ids),
                mock.call(line2, mock_cols_and_extractors, mock_cols_and_code_ids),
                mock.call(line3, mock_cols_and_extractors, mock_cols_and_code_ids)
            ]
        assert service.collection.count_documents({}) == 3
        assert list(service.collection.find({"name": "Alice"}, projection={"_id": False})) == [{"name": "Alice"}]
        assert list(service.collection.find({"name": "Bob"}, projection={"_id": False})) == [{"name": "Bob"}]
        assert list(service.collection.find({"name": "Cody"}, projection={"_id": False})) == [{"name": "Cody"}]


def test_process_header(service: EAContactsService, mock_code_cache: Mock) -> None:
    # Test various cases of _process_header.

    # First try with all fields.
    mock_code_cache.resources["Activist Code1"] = ActivistCode(id=1, name="Activist Code1")
    mock_code_cache.resources["Activist Code2"] = ActivistCode(id=2, name="Activist Code2")
    cols_and_extractors, cols_and_code_ids = service._process_header(
        "VANID\tFirst\tLast\tNoCall\tActivist_Code1_(Committee)\tNoEmail\tActivist_Code2_(Committee)\tExtraneous\t"
    ...
    cols_and_extractors, cols_and_code_ids = service._process_header("VANID\tFirst\tLast\tActivist_Code1_(Committee)")
    assert cols_and_extractors == [
        (0, ("van", int)),
        (1, ("first", br.identity)),
        (2, ("last", br.identity))
    ...
    cols_and_extractors, cols_and_code_ids = service._process_header("VANID\tFirst\tLast")
    assert cols_and_extractors == [
        (0, ("van", int)),
        (1, ("first", br.identity)),
        (2, ("last", br.identity))
    ...
    with pytest.raises(
        ValueError, match=r"Could not find the following activist codes in EveryAction: Activist_Code3, Activist_Code4"
    ):
        service._process_header(
            "VANID\tFirst\tLast\tActivist_Code1_(Committee)\tActivist_Code2_(Committee)\tActivist_Code3_(Committee)\t"
...
def test_collection(service: EAContactsService) -> None:
    ...
    assert service.collection is service._mongo[service._db_name][service._coll_name]


def test_find(service: EAContactsService, alice_doc: JSONType, bob_doc: JSONType) -> None:
    # Test that find actually finds the correct documents based on various queries.

    # Use another Alice doc to test find on first name when multiple people have the same first name.
    alice_doc2 = {
        "van": 3,
        "first": "Alice",
        "last": "Alice",
    ...
    service.collection.insert_many([copy.deepcopy(alice_doc), copy.deepcopy(alice_doc2), copy.deepcopy(bob_doc)])
    ...
    with pytest.raises(ValueError, match=r"At least one argument must be specified for find\."):
        service.find()


def test_get(service: EAContactsService, alice_doc: JSONType, bob_doc: JSONType) -> None:
    # Test that get can find a contact with the given VAN ID.
    service.collection.insert_many([copy.deepcopy(alice_doc), copy.deepcopy(bob_doc)])
...
def test_install(service: EAContactsService) -> None:
    ...
    with mock.patch.object(service, "_init_indexes", autospec=True) as mock_init_indexes:
        service.install()
...
def test_update(
    ...
    # Check that update updates a contact with new data or creates a contact if it could not be found.

    assert service.update(alice_update) == alice_doc
    assert service.get(1) == alice_doc  # Check that document is actually in collection.
    ...
    alice_update.del_emails = ["alice1@alice.com", "alice2@alice.com", "alice3@alice.com"]
...
def test_update_many(
    ...
    # Check that update_many allows many documents to be created/updated at once.
